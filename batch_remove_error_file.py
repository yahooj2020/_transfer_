    ...
    for item in execfile_list:
        try:
            ...
            if single_error_file in error_file_list:
                ...
                prefile_htmlfile,prefile,nextfile_htmlfile,nextfile = re_previous_next(item)
                prefile_path_htmlfile = os.path.join(templatePath,prefile_htmlfile)
                nextfile_path_htmlfile = os.path.join(templatePath,nextfile_htmlfile)
                single_error_file = single_error_file.split("-")[-1]



                if os.path.exists(item) and os.path.exists(prefile_path_htmlfile)  and os.path.exists(nextfile_path_htmlfile):
                    replace(prefile_path_htmlfile, single_error_file, nextfile)
                    replace(nextfile_path_htmlfile, single_error_file, prefile)
                    # remove_file(item)
        except:
            print(item)
